...
69. **Asserting sign(jvp) == sign(FD) per pinned species in
    tests/test_condensation_live_tp.py.** RETRACTED as a test of the tangent:
    it measures the finite-difference step size. Sweeping dT on the S8 gas
    column gave +8.99e14 (dT=4), +1.47e15 (2), +2.35e15 (1), -4.87e13 (0.5),
    +6.32e15 (0.25), +1.71e16 (0.125) -- FD ~ 1/dT, i.e. a fixed-size
    discontinuity over 2*dT, not a converging difference quotient (the
    `fix_species` pin captures the column at a discrete accepted step, and the
    accepted-step count bounces 2476-2820 over +/-4 K with no trend). The
    dT=0.5 the test used sits on a near-cancellation, which is where the sign
    flipped; the original agreement was luck of the step size, not a
    regression -- no VULCAN-JAX or retrieval commit since the test was written
    touches this path (65a334e's Ros2 escape term is gated on a non-empty
    `diff_esc`; W39b.yaml has `diff_esc: []`). Replaced by the CONSERVED
    RESERVOIR (gas + condensate), which is step-stable over the same 32x dT
    range (-1.21e14 to -1.34e14) and which the tangent reproduces to 18-22%.
    Corollary: the "0.91 relative" figure quoted for this state is one sample
    of a step-size-dependent quantity; the durable claim is order-unity wrong,
    and it stays a refusal. -- 2026-07-29; sweep in the test's own comment.
